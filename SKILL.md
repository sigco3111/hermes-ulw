...
> **Runtime**: ULW runs **on OpenCode** (the only supported runtime). Install via `bunx oh-my-openagent install`. Other CLIs (Codex, Claude Code, etc.) do not have ULW — this skill assumes an OpenCode + oh-my-openagent environment.
...
This installs oh-my-openagent as an OpenCode plugin, adding:
...
> **Note**: ULW is an **OpenCode-only** feature. There is no Codex / Claude Code equivalent. If you need a similar "drive the keyword" pattern in other CLIs, use the **Hermes-side** `ulw-routing-in-hermes` skill (software-development category) — it encodes the decision tree for *Hermes itself* (not OpenCode) handling a `ulw` request.
